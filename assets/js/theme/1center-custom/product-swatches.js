...
export class GetProductOptions {
  ...
  fetchProductsOptionsGQL() {
    ...
    this.getGraphQlData(allProdIDs, this.start, this.optionNextPage, this.variantNextPage);
  }


  getGraphQlDataOld(ids, start) {
    let i = 0
  ...
  getGraphQlData(ids, start, optCursor, variantCursor) {
    let i = 0
    let getOptionsQuery = `query getProductOptions($productIds: [Int!] = []) {
      site {
        products (first: 50, entityIds: $productIds${start}) {
          ...
          edges {
            node {
              ...
              name
              path
              productOptions(first: 50${optCursor}) {
                pageInfo{
                  hasNextPage
                  hasPreviousPage
                  startCursor
    ...
    fetch('/graphql', {
      ...
      // res.data.site.products.edges.map(prod => prod.node && prod.node.productOptions?.edges?.length > 0 && ((this.finalData[prod.node.entityId] = prod.node.productOptions.edges, this.productURLs[prod.node.entityId] = prod.node.path)));

      productData.edges.map(prod => {
        if (prod.node && prod.node.productOptions?.edges?.length > 0) {
          this.finalData[prod.node.entityId] = prod.node.productOptions.edges;
          this.productURLs[prod.node.entityId] = prod.node.path;
          if (prod.node.productOptions.pageInfo.hasNextPage) {
            this.allSwatch[prod.node.entityId] = null;
          }else{
            prod.node.productOptions.edges.length && prod.node.productOptions.edges.forEach(el => {
              let n = el.node;
              
              if($.trim(n.displayStyle).toLowerCase() == "swatch" && $.trim(n.displayName).toLowerCase() == "color" && n?.values?.pageInfo?.hasNextPage){
                this.allSwatch[prod.node.entityId] = null;
              }
            })
          }
      ...
      if (productData.pageInfo.hasNextPage) {
        this.start = `, after: "${productData.pageInfo.endCursor}"`;
        this.optCursor = `, after: "${productData.pageInfo.endCursor}"`;
        this.getGraphQlData(ids, this.start);
      } else {
        this.getAllSwatch();
        // this.makeHTMLDataForProductOptions()
      }
    });
  }
  ...
  getAllSwatch() {
    let reqArr = [];
    if (Object.keys(this.allSwatch).length) {

      Object.keys(this.allSwatch).forEach(pid => {

        reqArr.push(new Promise((resolve, reject) => {
          utils.api.product.getById(pid, { template: '1center-custom/getProductSwatch' }, (err, response) => {
            if (err) {
              reject(err);
            }
            resolve({pid:pid,count:parseInt($.trim(response), 10)});
          });
        }))
    ...
    Promise.allSettled(reqArr).then(data =>{
      data.length && data.forEach(k => {
        if(k.status == "fulfilled"){
          this.allSwatch[k.value.pid] = {}
          this.allSwatch[k.value.pid] = { count: k.value.count};
        }
      })
      this.makeHTMLDataForProductOptions();
    }).catch(err=>{
      ...
      if ($targetEl.length > 0) {
        value.forEach(opt => {
          ...
          if (data.values?.edges?.length > 1 && data.displayStyle.toLowerCase() === 'swatch') {
            ...
            let totalSwatch = this.allSwatch[key] ? this.allSwatch[key]['count'] : data.values?.edges?.length;
            moreSwatch = totalSwatch - 4;
            ...
            data.values?.edges?.slice(0, 4).forEach(option => {
              ...
              innerValuesHTML += `
                <li class="form-option-wrapper">
                  <a href="${this.productURLs[key] ? this.productURLs[key] : '#'}" data-product-attribute-id="${opt.node.entityId}" data-product-attribute-value="${data.entityId}" aria-label="Swatches">
